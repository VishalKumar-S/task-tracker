...
@Singleton
class TaskRepository @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext)
  ...
  def findDueTasks(now: LocalDateTime): Future[Seq[Task]] = {
    ...
    val query = tasks.filter(t => t.dueDate >= now && t.dueDate <= in10Minutes && !t.notified)
    db.run(query.result)
